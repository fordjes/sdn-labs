...
0. This is the code that makes a learning L2 switch
    ...
    ![Alta3 Research Wireshark Openflow Column Setup](https://alta3.com/labs/images/alta3_sdn_errno98.png)

0. Make sure no process is hogging port TCP 6653.

  `student@beachhead:~$` `sudo ss -plant | grep 6653`
    ```  ubuntu@beachhead:~$ sudo ss -plant | grep 6653
    LISTEN     0      10           *:6653                     *:*                   users:(("ovs-testcontrol",pid=10033,fd=3))
    ```
    >
    If a process is running, kill it with: `sudo kill <PID>` where PID is the process ID.
