...
This command shows the expected internal IP of 10.0.0.4:
`$` `ip a`

We can start a continuous ping towards RED2 at 10.0.0.5:
`$` `ping 10.0.0.5`

Now let's jump into compute host1. The first thing that we need is the Neutron port ID for VM RED1:
`[root@compute1 ~(keystone_demo)]#` `nova interface-list RED1`
We can see the port ID from this command. It is 32 characters, however, we'll only need the first 3 digits. With this info, we can find the Linux bridge in front of RED1:

`[root@compute1 ~(keystone_demo)]#` `brctl show`
This will show the three Linux bridges present in Koopa. One exists for each VM in this host. Note that the first bridge in the list matches the first ten digits of the Port ID.

This first bridge has two ports. The top port leads to the VM's virtual NIC. The QVE port is one end of a veth pair. The other end is on the integration bridge. 

Lets do a packet capture on the tap interface to see ICMP traffic.
`[root@compute1 ~(keystone_demo)]#` `tcpdump -1 (first port number) -n -e icmp`
The "icmp" command includes a filter for icmp traffic. The "-e" command filters in layer 2 information to see MAC addresses. The "-n" command disables any attempt of resolution of domain names or OUIs or MAC addresses.

You will see ICMP requests and replies between Koopa and Goomba. We'll use this information later.

Return to the tap interface on the Linux bridge. The tap interface is where security group rules are implemented. The next part of our lab will explore how security group rules look in IP tables:

`[root@compute1 ~(keystone_demo)]#` `iptables - S | grep o (first three digits of Koopa's port ID)`

"-s" will reveal all rules. Filtering with "grep" will search for only rules associated with Koopa's port ID. The letter "o" before the port ID is just the outbound policy.

Find the chain to the right of the inbound tag (physdev-in). This tag means that the chain is leaving or egress from the VM. Now find some of the security rules written into the code.

Find the line that contains "Allow DHCP client traffic." This is required to allow the initial spinup of the instance. Another rule prevents DHCP spoofing, which is contained in the line that includes "Prevent DHCP Spoofing by VM"

Let's take a closer look at this chain by typing it out:
`[root@compute1 ~]#` `iptables -S neutron-openvswi-s0e6fb773-9m 

This rule only allows traffic from Koopa's allocated IP (which you can find in the code). It also ensures that traffic comes from the assigned MAC address for Koopa, "Allow traffic from defined IP/MAC pairs."

Next we will return to the column security chain:
`[root@compute1 ~]#` `iptables -s neutron-openvswi-sg-chain` 

The ICMP traffic is forwarded via a Linux bridge to open vSwitch, specifically the integration bridge. Let's find the associated port in Open vSwitch.

Back on Compute Host 1, type the following:
`[root@compute1 ~(keystone_demo)]#` `ovs-vsctls show`
This shows the integration bridge and its ports.

To know what happens after ICMP flow through the intergration bridge on the OVS port, you have the check OpenFlow rules.
`[root@compute1 ~(keystone_demo)]#` `watch -n.5 "ovs-ofctl dump-flows br-int \ 
> table=0 | grep -v n_packets=0"` 
This command will break this out. The "ovs-ofctl dump-flows br-int" command will show the OpenFlow entries and the flow entries for the integration bridge. To make it easier to observe, we are filtering down to just the first table in the processing pipeline, "table=0." 

We will also ignore flow entries with no hits at all, hence the command "grep -v n_packets=0." The watch command "watch -n.5", which is set to half-second intervals, will allow us to easily show which flow entries have their counters incremented.

Another command will show us the forwarding table of the integration bridge.
`[root@compute1 ~(keystone_demo)]#` `ovs-appctl fdb/show br-int`

By typing the command below, we can see the name of the ovs port that port 4 maps to.
`[root@compute1 ~(keystone_demo)]#` `ovs-ofctl show br-int`

Before moving on, there is another Linux bridge to observe. 
`[root@compute1 ~(keystone_demo)]#` `brctl show`
This will show you the bridges- let's look at the second bridge.

Let's look at iptables one more time to observe ingress rules for the VM Goomba.
`[root@compute1 ~(keystone_demo)]#` `iptables -S | grep i399`
This command shows the security chain for inbound flows to Goomba.

Here you will learn that TCP port 22 traffic is permitted, but only for an IP set with this ID number. The "ipset list" command reveals the numbers of this set:
`[root@compute1 ~(keystone_demo)]#` `ipset list`

In the next scenario, we'll follow the flow when your machines are on the same network, but have different hosts. Koopa must reach Red3. 

Let's start by following some ICMP traffic from Koopa to Red3. We'll start with Koopa's CLI and ping across to Red 3.
`$` `$ ping 10.0.0.6`
This shows that the connection works before we get any farther.

`[root@compute1 ~]#` `ovs-appctl fdb/show br-int` 
This shows the forwarding table for the integration bridge.

`[root@compute1 ~]#` `ovs-ofctl show br-init`
This command will reveal which port is br-tun, the next connection in the flow.

`[root@compute1 ~]#` `ovs-vsctl show`
This shows that br-int has a patch port called patch-tun. This is a direct link to br-tun.

Lets look at the OpenFlow port number for br-tun:
`[root@compute1 ~]#` `ovs-ofctl show br-tun`
Here we see patch-int maps to OpenFlow port 1. Also, we can see ports 4 and 5 map to interfaces used for VXLan tunnels.

So now lets check OpenFlow entries on the tunneling bridge. We can start with the first table, which is table zero:
`[root@compute1 ~]#` `watch -n.5 "ovs-ofctl dump-flows br-tun table=0"`
The first flow entry says that if traffic comes in on OpenFlow port `, resubmit or move it to OpenFlow table 1. So all traffic coming from the integration bridge will go to table 1. 

Lets check that table:
`[root@compute1 ~]#` `watch -n.5 "ovs-ofctl dump-flows br-tun table=1"`
Here all the tap rules relate to the distributed routers so none of those will match. A default last line here will match which goes to table 2, so lets look at that table:
