...
0. Since we just deleted the data-paths we created, the *pingall* command should fail once again.
...
0.
  ...
  ```
  NXST_FLOW reply (xid=0x4):
  cookie=0x5, duration=9.017s, table=0, n_packets=0, n_bytes=0, idle_age=9, in_port=1 actions=output:3
  cookie=0x5, duration=4.255s, table=0, n_packets=0, n_bytes=0, idle_age=4, in_port=3 actions=output:1
  ```

0. Run the *pingall* command to help reveal the data paths we just set up.
...
0. Answer the following questions about our current environment:
