...
0. Without closing the first terminal (*Terminal1*), open a second terminal (*Terminal2*). In *Terminal2* type the following:

    `student@beachhead:/$` `ryu-manager ~/base.py`

0. Without closing the first terminal (*Terminal1*), or the second terminal (*Terminal2), open a third terminal (*Terminal3*). In *Terminal3* type the following:
